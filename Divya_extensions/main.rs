...
fn main() {
    ...
    let mut last_time = command_time;
    ...
    loop {
        ...
        let start = Instant::now();
        let (status, highlight) = handle_command(&mut sheet, trimmed, &mut sleep_time);
        ...
        sleep_time = 0.0;
        if let Some((target, parents, children)) = highlight {
            sheet.print_spreadsheet_with_highlights(target, &parents, &children);
        } else {
            sheet.print_spreadsheet();
        }
